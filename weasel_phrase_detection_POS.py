...
pos_BIO_count = {} # Dict in the format {(word, key):[B, I, O]}
EmissionProbability = {}
...
def preprocessFile():
	...
	w = relabelFile(f)
	calculateCount()
	global pos_BIO_count
	pos_BIO_count = goodTuring(pos_BIO_count)
	calculateInitialState(pos_BIO_count)
	calculateEmissionProbability(pos_BIO_count)
	pos_BIO_count = None
...
def calculateCount():
	...
	for line in f:    
	    ...
	    if pos not in pos_BIO_count:
	      	pos_BIO_count[pos] = np.array([0,0,0])
	    addBIOCount(pos_BIO_count[pos],cue)  	
	f.close()
